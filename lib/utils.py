...
def scaled_Laplacian(W):
    ...
    lambda_max = np.linalg.eig(L)[0].real
...
def get_attn_pad_mask(seq_q, seq_k, node_num, dimc):
    new_seq_q = torch.zeros(len(seq_q), node_num).to(device)
    new_seq_k = torch.zeros(len(seq_q), node_num).to(device)
